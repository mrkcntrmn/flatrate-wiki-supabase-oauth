...
test("R3D: count()+1 allocator removed from live provisioner", async () => {
  ...
  assert.match(provisioner, /forum_tech_number_required/);
...
test("R3D: linked-user check precedes tech_number requirement", async () => {
  ...
  const techIdx = provisioner.indexOf("TechNumberPayload::parseOptional");
  const requiredIdx = provisioner.indexOf("forum_tech_number_required");
...
test("R3D: TechNumberPayload enforces strict JSON integers", async () => {
  const payload = await text("src/Identity/TechNumberPayload.php");
  assert.match(payload, /MAX_SAFE_INTEGER = 9007199254740991/);
  assert.match(payload, /is_int\(\$value\)/);
  assert.match(payload, /forum_tech_number_required/);
...
test("R3D: cross-repo contract fixture agrees on field and codes", async () => {
  ...
  assert.equal(fixture.missing.error, "forum_tech_number_required");
  assert.equal(fixture.invalid.status, 400);
  assert.equal(fixture.invalid.error, "invalid_tech_number");
  assert.deepEqual(fixture.invalid_examples, [0, -1, 1.2, "20031", "tech_20031", null]);
});
