...
(str_contains($forumLess, 'li:has(> .Post-controls)')
    ...
    && str_contains($forumLess, '.item-flatRateReplied')
    && str_contains($forumLess, '.Post-mentionedBy-summary')
    && str_contains($pv, "items.has('replies')")
    && str_contains($pv, "items.remove('replies')")
    && str_contains($pv, "items.add('flatRateReplied'")
